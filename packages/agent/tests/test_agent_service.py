...
def test_task_command_defaults() -> None:
    from r2flow_agent.service import TASK_NAME, task_command

    cmd = task_command("C:/py/python.exe")
    assert "-m r2flow_agent.service run" in cmd
...
def test_task_command_prefers_pythonw(tmp_path: Path) -> None:
    from r2flow_agent.service import task_command
...
def test_missing_config_fails_service_start(
    ...
    import r2flow_agent.config as cfg
    import r2flow_agent.service as svc
