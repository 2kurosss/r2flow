...
from r2flow_agent.client import OrchestratorClient
from r2flow_agent.executor import (
    ProcessExecutor,
    _flow_run_args,
    _safe_extract,
    _verify_pack,
...
async def test_deploy_pack_extracts_and_drops_stale_files(
    ...
    proc_dir = await executor.deploy(process_id, {}, ["r2flow-engine"], pack_data=first)
    ...
    await executor.deploy(process_id, {}, ["r2flow-engine"], pack_data=second)
...
def test_pack_requirements_injects_engine(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("R2FLOW_PACK_REQUIREMENT", raising=False)
    assert agent_main._pack_requirements([]) == ["r2flow-engine[windows]"]
    assert agent_main._pack_requirements(["r2flow-engine"]) == ["r2flow-engine"]
    assert agent_main._pack_requirements(["requests"]) == ["requests", "r2flow-engine[windows]"]


def test_pack_requirements_respects_override(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("R2FLOW_PACK_REQUIREMENT", "")
    assert agent_main._pack_requirements([]) == []
    monkeypatch.setenv("R2FLOW_PACK_REQUIREMENT", "r2flow-engine==0.8.4")
    assert agent_main._pack_requirements(["requests"]) == ["requests", "r2flow-engine==0.8.4"]
...
async def test_run_flow_invokes_engine_runner(
    ...
    assert captured["argv"][1:] == ["-m", "r2flow.run_flow", "01_notepad_flow.json"]
