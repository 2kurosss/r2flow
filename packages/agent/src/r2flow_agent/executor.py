...
#: Pack delivery format — mirrors the engine's ``r2flow.pack``.
...
def _verify_pack(directory: Path) -> list[str]:
    ...
    if not manifest_path.is_file():
        return [f"no {PACK_MANIFEST} — not a r2flow pack"]
    try:
...
def _flow_run_args(proc_dir: Path) -> list[str]:
    """Arguments for ``python -m r2flow.run_flow`` selecting the pack's flow.
...
class ProcessExecutor:
    ...
    async def run(
        ...
        *,
        ...
        env:
            Extra environment entries (cloud coordinates, ``R2FLOW_ASSET_*``
    ...
    async def run_flow(
        ...
        *,
        ...
        """Run a pack's flow with the engine (``python -m r2flow.run_flow``).
        ...
        python_exe = self._venv_python(proc_dir)
        logger.info(
            "Running pack flow %s: %s -m r2flow.run_flow %s",
            process_id,
            python_exe,
            " ".join(args),
        )
        return await self._spawn(
            process_id, [str(python_exe), "-m", "r2flow.run_flow", *args], proc_dir, env
        )
