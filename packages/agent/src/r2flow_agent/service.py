...
from r2flow_agent.config import CONFIG_PATH, load_config, save_config

logger = logging.getLogger("r2flow_agent")

TASK_NAME = "R2FlowAgent"
...
def run_forever() -> None:
    ...
    from r2flow_agent.main import run_agent

    cfg = load_config()
    if not cfg.get("orchestrator_url"):
        print(
            f"No agent config at {CONFIG_PATH} — run `r2flow-agent-service install` first",
            file=sys.stderr,
        )
        sys.exit(2)
...
def task_command(
    ...
    parts = [
        ...
        f"{_ps_quote(python_exe)} -Argument '-m r2flow_agent.service run' "
...
def update_package() -> bool:
    """Upgrade r2flow-agent in the current venv and restart the task.
    ...
    from r2flow_agent.client import agent_version

    print(f"Upgrading r2flow-agent (interpreter: {sys.executable})...")
...
def task_status() -> str:
    ...
    if cfg.get("orchestrator_url"):
        ...
        lines.append("agent:   (no config - run `r2flow-agent-service install`)")
    ...
    try:
        res = _run_powershell(
            ...
            "CommandLine LIKE '%r2flow_agent.service%'\" "
...
def main(argv: list[str] | None = None) -> None:
    ...
    sub.add_parser("update", help="upgrade r2flow-agent from PyPI and restart the task")
    ...
    if args.command == "install":
        ...
        if install_task(args.user):
            ...
            print("Start it now with: r2flow-agent-service start")
...
def _auto_url(orchestrator_url: str) -> str:
    from r2flow_agent.config import agent_url_for
