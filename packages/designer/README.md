...
Visual flow editor for the [r2flow](https://github.com/2kurosss/r2flow) RPA engine.

Drag nodes onto the canvas, connect them, configure tool selectors, and debug
the flow step by step — in the browser, against the local r2flow engine.
...
- **Visual canvas** — nodes for control flow (start/end/if/loop/fail), variables
  (`set` node), subflows (`flow` node), and every registered r2flow tool;
...
its `windows` extra: `pip install "r2flow-engine[windows]"`. For **Record →
flow** also install the recorder extra: `pip install "r2flow-designer[record]"`.
...
# or:
python -m r2flow_designer flow.json
```

The server binds to `127.0.0.1:8756` and serves the prebuilt bundle from
`designer-web/dist` (or `src/r2flow_designer/static` in installed packages).

## Flow file

The flow document format is versioned — see the
[flow format contract](https://github.com/2kurosss/r2flow#flow-format-v2)
in the r2flow repo. Current version: **v2**.
...
## Publishing a flow to r2flow-cloud

A flow becomes a **pack** (`r2flow-pack-v1`) and is pushed to a
[r2flow-cloud](https://github.com/2kurosss/r2flow) orchestrator. The
...
python -m r2flow_designer.publish flow.web.json \
