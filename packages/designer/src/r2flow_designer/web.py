"""Web designer backend: local FastAPI server for the R2Flow flow editor.
...
def _validate_flow(data: Any, registry: ToolRegistry | None = None) -> dict[str, Any]:
    ...
    engine's :func:`r2flow.flow.validate_document`; only HTTP-shape
...
def create_app(flow_path: Path) -> FastAPI:
    ...
    app = FastAPI(title="r2flow-designer", docs_url=None, redoc_url=None)
    registry = _registry()
    ...
    @app.post("/api/record/discard")
    async def record_discard() -> dict[str, str]:
        try:
            await asyncio.to_thread(recorder.discard)
        except RecordError as exc:
            raise HTTPException(status_code=409, detail=str(exc)) from exc
    ...
    if static is not None:
        ...
        def no_bundle() -> PlainTextResponse:
            return PlainTextResponse(
                ...
                "Then restart python -m r2flow_designer"
            )
