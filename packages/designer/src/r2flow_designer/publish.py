"""Publish a flow project to a r2flow-cloud orchestrator as a pack.
...
* :func:`r2flow.pack.publish_pack` builds the manifest, zips and uploads to
...
Usage:
    python -m r2flow_designer.publish flow.web.json \
        --url http://localhost:8000 --token r2f_... \
...
from r2flow.pack import publish_pack
...
def publish(
    ...
    *,
    ...
    Raises:
        ...
        r2flow.core.errors.InvalidInput: On pack build/upload failure.
    """
    root = Path(directory)
    if not root.is_dir():
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="r2flow_designer.publish", description=__doc__)
    ...
    parser.add_argument("--token", required=True, help="API token (r2f_...) or user JWT")
