...
from r2flow.windows.tools._resolve import build_selector, resolve_element
from r2flow.windows.tools.screenshot import _pil_format
from r2flow.windows.tools.selector_capture.cli import main
from r2flow.windows.tools.selector_capture.generate import build_wait_config
...
class TestToolValidation:
    ...
    async def test_keyboard_maps_unknown_key_to_invalid_input(self) -> None:
        ...
        with (
            patch(
                "r2flow.windows.tools.keyboard._send",
...
class TestCliDispatch:
    def test_single_dispatches_with_tool_defaults(self, tmp_path: Any) -> None:
        from r2flow.windows.tools.selector_capture.generate import ToolType

        out = str(tmp_path / "sel.json")
        with patch("r2flow.windows.tools.selector_capture.cli.run_single_mode") as run:
            main(["single", "-o", out])
    ...
    def test_series_dispatches_without_tool_arg(self, tmp_path: Any) -> None:
        ...
        with patch("r2flow.windows.tools.selector_capture.cli.run_series_mode") as run:
            main(["series", "-o", out])
    ...
    def test_record_dispatches_without_tool_arg(self, tmp_path: Any) -> None:
        ...
        with patch("r2flow.windows.tools.selector_capture.cli.run_record_mode") as run:
            main(["record", "-o", out])
        run.assert_called_once_with(output=out)
