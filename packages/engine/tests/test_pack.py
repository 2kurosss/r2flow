"""Tests for r2flow.pack — build/verify manifests and run_flow --pack."""
...
class TestBuildVerify:
    def test_build_and_verify_roundtrip(self, tmp_path: Path) -> None:
        ...
        assert manifest["schema"] == "r2flow-pack-v1"
...
class TestPackCli:
    def test_cli_build_and_verify(self, tmp_path: Path) -> None:
        ...
        verify = subprocess.run(
            [sys.executable, "-m", "r2flow.pack", "verify", str(root)],
            capture_output=True,
            text=True,
        )
...
class TestDelivery:
    ...
    def test_zip_slip_rejected(self, tmp_path: Path) -> None:
        ...
        with zipfile.ZipFile(evil, "w") as zf:
            ...
            zf.writestr(PACK_MANIFEST, json.dumps({"schema": "r2flow-pack-v1"}))
...
class TestPublish:
    ...
    def test_push_local_zip_without_api_url(self, tmp_path: Path) -> None:
        ...
        assert "R2FLOW_API_URL" in result.stderr
    ...
    def test_push_uploads_to_orchestrator(self, tmp_path: Path) -> None:
        ...
        server = self._fake_orchestrator({"/packs/p/versions/1.0.0": (201, b"")})
        base = f"http://127.0.0.1:{server.server_port}"
        env = {**os.environ, "R2FLOW_API_TOKEN": "tok"}
        try:
            result = subprocess.run(
                [
                    sys.executable,
