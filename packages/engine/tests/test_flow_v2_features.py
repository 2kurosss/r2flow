...
from r2flow.core.assets import EnvAssetProvider
from r2flow.core.errors import BusinessError, ElementNotFound, InfrastructureError
...
from r2flow.flow import FlowError, FlowRunner, validate_document
...
class TestAssets:
    ...
    async def test_asset_secret_redacted_in_log(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("R2FLOW_ASSET_TOKEN", "topsecret")
    ...
    async def test_unknown_asset_fails_node(self) -> None:
        runner = FlowRunner(_registry(), assets=EnvAssetProvider(prefix="R2FLOW_NOPE_"))
        doc = _doc(
            [
                {"id": "s", "kind": "start", "config": {}},
...
class TestSelectorKeys:
    ...
    async def test_missing_key_raises_in_production(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(FlowError, match="R2FLOW_DEV_CAPTURE"):
            await runner.run(doc)
    ...
    async def test_missing_key_captured_in_dev_mode(
        ...
        monkeypatch.setattr(
            "r2flow.windows.tools.selector_capture.capture_once_async", fake_capture
        )
