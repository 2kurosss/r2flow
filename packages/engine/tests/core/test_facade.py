"""Tests for r2flow.facade — R2Flow facade class and ProcessHandle."""
...
from r2flow.facade import ProcessHandle, R2Flow
...
class TestR2FlowInit:
    def test_empty(self) -> None:
        bot = R2Flow()
        assert bot._registry.list_tools() == []

    def test_with_tools(self) -> None:
        bot = R2Flow(tools=[StubTool(tool_name="a"), StubTool(tool_name="b")])
...
class TestR2FlowProcess:
    @pytest.mark.asyncio
    async def test_process_returns_handle(self) -> None:
        bot = R2Flow(tools=[ProcessStub()])
...
class TestR2FlowClick:
    @pytest.mark.asyncio
    async def test_click_with_handle(self) -> None:
        bot = R2Flow(tools=[ClickStub()])
...
class TestR2FlowGetElement:
    @pytest.mark.asyncio
    async def test_get_element_with_handle(self) -> None:
        output = {"element": {"name": "OK"}}
        bot = R2Flow(tools=[StubTool(tool_name="windows.get_element", output=output)])
        handle = ProcessHandle(pid=42, name="app")
...
class TestR2FlowCall:
    @pytest.mark.asyncio
    async def test_call_custom_tool(self) -> None:
        bot = R2Flow(tools=[StubTool(tool_name="custom", output="done")])
