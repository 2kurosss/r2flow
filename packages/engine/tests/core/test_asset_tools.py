...
from r2flow.core.assets import EnvAssetProvider, HttpAssetProvider, asset_provider_from_env
...
class TestAssetTools:
    ...
    async def test_get_text_asset(self, monkeypatch: Any) -> None:
        monkeypatch.setenv("R2FLOW_ASSET_CRM_URL", "https://crm.example")
    ...
    async def test_credential_fields(self, monkeypatch: Any) -> None:
        monkeypatch.setenv("R2FLOW_ASSET_CRM_LOGIN", "alice")
        monkeypatch.setenv("R2FLOW_ASSET_CRM_PASSWORD", "s3cret")
...
class TestSecretHandling:
    ...
    async def test_value_is_redacted_and_excluded_from_results(self, monkeypatch: Any) -> None:
        monkeypatch.setenv("R2FLOW_ASSET_CRM_PASSWORD", "topsecret")
...
class TestProviderSelection:
    ...
    def test_factory_prefers_orchestrator(self, monkeypatch: Any) -> None:
        monkeypatch.setenv("R2FLOW_ORCHESTRATOR_URL", "http://127.0.0.1:9")
        monkeypatch.setenv("R2FLOW_AGENT_ID", "a1")
        monkeypatch.setenv("R2FLOW_AGENT_TOKEN", "t")
        assert isinstance(asset_provider_from_env(), HttpAssetProvider)

    def test_factory_falls_back_to_env(self, monkeypatch: Any) -> None:
        for key in ("R2FLOW_ORCHESTRATOR_URL", "R2FLOW_AGENT_ID", "R2FLOW_AGENT_TOKEN"):
