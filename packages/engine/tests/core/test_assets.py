"""Tests for r2flow.core.assets — EnvAssetProvider + facade asset()."""
...
from r2flow.core.assets import DEFAULT_ASSET_PREFIX, EnvAssetProvider
from r2flow.core.errors import InvalidInput
from r2flow.core.events import ToolEvent
...
class TestEnvAssetProvider:
    ...
    def test_missing_asset_raises(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.delenv("R2FLOW_ASSET_NOPE", raising=False)
        with pytest.raises(InvalidInput, match="R2FLOW_ASSET_NOPE"):
...
class TestFacadeAsset:
    def test_default_provider(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("R2FLOW_ASSET_TOKEN", "abc")
        assert R2Flow().asset("token") == "abc"
    ...
    def test_custom_provider(self) -> None:
        ...
        assert R2Flow(assets=StaticProvider()).asset("x") == "value:x"
...
class TestFacadeSecretRedaction:
    ...
    async def test_asset_value_redacted_from_tool_events(self) -> None:
        ...
        bot = R2Flow(tools=[echo], assets=StaticProvider())
