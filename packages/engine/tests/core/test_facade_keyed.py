...
from r2flow.core.errors import ElementNotFound, InvalidInput
from r2flow.core.selectors import SelectorStore
from r2flow.core.tool import AbstractTool
from r2flow.facade import R2Flow
from r2flow.windows.tools.selector_capture.api import CapturedSelector
...
def _patch_capture(
    ...
    monkeypatch.setattr("r2flow.windows.tools.selector_capture.capture_once_async", fake_capture)
    return used
...
class TestKeyedConfig:
    ...
    async def test_stored_key_runs_silently(
        ...
        bot = R2Flow(tools=[tool], selector_store=store_path, dev_capture=True)
    ...
    async def test_missing_key_production_raises(
        ...
        used = _patch_capture(monkeypatch, [])
        tool = _StubTool("windows.click", [{"status": "clicked"}])
        bot = R2Flow(tools=[tool], selector_store=tmp_path / "s.json", dev_capture=False)
        with pytest.raises(InvalidInput, match="R2FLOW_DEV_CAPTURE"):
            await bot.click(key="submit")
        assert tool.calls == []
        assert used == []
    ...
    async def test_stale_selector_production_fails(
        ...
        bot = R2Flow(tools=[tool], selector_store=store_path, dev_capture=False)
...
class TestKeyedOtherTools:
    ...
    async def test_set_text_and_input_text_keyed(self, tmp_path: Path) -> None:
        ...
        bot2 = R2Flow(tools=[tool2], selector_store=store_path, dev_capture=True)
...
class TestDevCaptureEnv:
    def test_env_enables_dev_capture(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        monkeypatch.setenv("R2FLOW_DEV_CAPTURE", "1")
        bot = R2Flow(selector_store=tmp_path / "s.json")
        assert bot._dev_capture is True

    def test_explicit_beats_env(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        monkeypatch.setenv("R2FLOW_DEV_CAPTURE", "1")
        bot = R2Flow(selector_store=tmp_path / "s.json", dev_capture=False)
