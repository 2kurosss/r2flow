"""Tests for r2flow.windows.selector_rank + strict mode + ranked generation."""
...
from r2flow.windows.selector_rank import (
    RankedSelector,
    candidate_configs,
...
class TestStrictResolve:
    ...
    async def test_ambiguous_raises_invalid_input(self) -> None:
        from r2flow.windows.tools._resolve import resolve_element
...
class TestRankedGeneration:
    ...
    def test_inline_selector_translates_numeric_type(self) -> None:
        from r2flow.windows.tools.selector_capture.generate import build_inline_selector
...
class TestRankCapturedFallback:
    def test_rank_failure_returns_none(self) -> None:
        from r2flow.windows.tools.selector_capture.recorder import _rank_captured

        sel = BestSelector(control_type="Button", name="OK")
        with patch(
            "r2flow.windows.selector_rank.rank_best_selector",
