...
def _clean_allowlist_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("R2FLOW_ALLOWED_COMMANDS", raising=False)
...
class TestProcessAllowlist:
    ...
    def test_env_override(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("R2FLOW_ALLOWED_COMMANDS", "custom.exe, other.exe")
        tool = ProcessTool()
    ...
    def test_empty_env_denies_everything(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("R2FLOW_ALLOWED_COMMANDS", "")
        tool = ProcessTool()
    ...
    def test_constructor_beats_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("R2FLOW_ALLOWED_COMMANDS", "env-app.exe")
