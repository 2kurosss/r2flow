"""Tests for r2flow.windows.tools.image — find/click image fallback."""
...
from r2flow.windows.tools.image import ClickImageTool, FindImageTool, _find_on_screen
...
class TestFindImage:
    ...
    async def test_found(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        template.write_bytes(b"png")
        monkeypatch.setattr(
            "r2flow.windows.tools.image._find_on_screen",
    ...
    async def test_click_image(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        import r2flow.windows.tools.click as click_module
    ...
    async def test_sandbox_escape(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("R2FLOW_FILE_ROOT", str(tmp_path / "root"))
        (tmp_path / "root").mkdir()
