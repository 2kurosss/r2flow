...
The latest tagged release on PyPI (`r2flow-engine`) is supported.

## Reporting a vulnerability

Please report security issues privately via
[GitHub Security Advisories](https://github.com/2kurosss/r2flow/security/advisories/new)
...
r2flow is an RPA engine: **flow configs, tool configs and custom
...
- `windows.process` starts only executables matching the
  `R2FLOW_ALLOWED_COMMANDS` allowlist (or the built-in default list);
...
- Robot `config` files never hold secrets: `R2FLOW_ASSET_*` (and other
  framework `R2FLOW_*` settings) are excluded from the env overlay, so
  they cannot leak through `Config.to_dict()` / `repr()`.
- Screenshot and file paths can be confined with `R2FLOW_OUTPUT_ROOT` /
  `R2FLOW_FILE_ROOT`.
