...
Requires: pip install r2flow[windows]
...
real orchestrator queue (handy for post-mortem debugging after a failed run):
    ...
    R2FLOW_QUEUE=sqlite  local file bot-data/queue.db (default)
    R2FLOW_QUEUE=cloud   orchestrator queue (needs R2FLOW_API_URL,
                         R2FLOW_AGENT_ID and R2FLOW_TOKEN)
...
bot = R2Flow(tools=[ProcessTool(), ClickTool(), WaitTool()])
...
def make_queue() -> Queue:
    ...
    if os.getenv("R2FLOW_QUEUE", "sqlite").lower() == "cloud":
        return HttpQueue(
            os.environ["R2FLOW_API_URL"],
            agent_id=os.environ["R2FLOW_AGENT_ID"],
            token=os.environ["R2FLOW_TOKEN"],
        )
...
async def main() -> None:
    ...
    try:
        report = await run_transactions_async(
            ...
            run_id=os.getenv("R2FLOW_RUN_ID", str(CONFIG.robot.run_id)),
            on_init=dispatcher,
