"""HTTP queue backend speaking the r2flow-cloud transaction contract.
...
from r2flow.core.errors import InfrastructureError, InvalidInput
...
def _engine_version() -> str | None:
    """Installed r2flow-engine version (None when not resolvable).
    ...
    if _engine_version_cached is _ENGINE_VERSION_UNSET:
        try:
            _engine_version_cached = _pkg_version("r2flow-engine")
...
class HttpQueue:
    """Queue backend over the r2flow-cloud HTTP contract.

    *base_url* is the API root including the version/prefix path,
    e.g. ``"https://host/api"`` for a default r2flow-cloud deployment.
    ...
    def claim(self, queue: str, *, run_id: str, lease_seconds: int = 300) -> ClaimedItem | None:
        ...
        agent_version = os.environ.get("R2FLOW_AGENT_VERSION")
        if agent_version:
