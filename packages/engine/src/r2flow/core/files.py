...
Sandbox: when ``R2FLOW_FILE_ROOT`` is set, every path must resolve
...
ENV_FILE_ROOT = "R2FLOW_FILE_ROOT"
...
def confine_path(path: Path, *, env_var: str = ENV_FILE_ROOT) -> Path:
    ...
    default sandbox is ``R2FLOW_FILE_ROOT``; screenshot output reuses
    this helper with ``R2FLOW_OUTPUT_ROOT``.
    """
