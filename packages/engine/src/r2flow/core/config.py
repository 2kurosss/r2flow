...
from r2flow.core.errors import ConfigError

_MISSING = object()

#: Framework-owned ``R2FLOW_*`` settings that are consumed directly by
...
#: Prefixes of framework ``R2FLOW_*`` namespaces that must never be
...
def _apply_env_overlay(document: dict[str, Any], prefix: str) -> None:
    ...
    ``R2FLOW_ROBOT__QUEUE`` sets ``robot.queue``; double underscore nests,
    single underscores stay literal (``R2FLOW_ROBOT_NAME`` → ``robot_name``).
    Values are typed with TOML scalar syntax (ints, bools, quoted strings).
    Framework-owned settings (``R2FLOW_BLOCKING_TIMEOUT``,
    ``R2FLOW_ALLOWED_COMMANDS``) are skipped — they configure the engine,
...
def load_config(
    ...
    *,
    ...
    When *env_prefix* is set (default ``\"R2FLOW_\"``), matching env vars
    override file values — per-environment tweaks without editing TOML.
    ``R2FLOW_ROBOT__QUEUE`` sets ``robot.queue`` (``__`` nests, values are
