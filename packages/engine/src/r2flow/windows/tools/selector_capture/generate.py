...
that the corresponding ``r2flow.windows.tools`` tool expects.
...
class ToolType(enum.StrEnum):
    ...
    Attributes:
        ...
        KEYBOARD: Press keys / hotkeys (no selector).
        DELAY: Fixed pause between recorded steps.
    """
    ...
    def needs_duration(self) -> bool:
        ...
        return self in (ToolType.WAIT, ToolType.DELAY)
...
class GenerateParams:
    ...
    Attributes:
        ...
        duration_ms: Duration (milliseconds) for the ``wait``/``delay`` tools.
        keys: Key combo for the ``keyboard`` tool (e.g. ``"[ENTER]"``).
        button: Mouse button for ``click`` (``left``/``right``).
        clicks: Single (1) or double (2) click.
    """
...
def generate_nodes(
    ...
    Mapping:
        ...
        * ``Wait`` → ``[windows.wait]``
        * ``Keyboard`` → ``[windows.keyboard]`` (no selector)
        * ``Delay`` → ``[windows.delay]`` (replay pacing)

    Args:
        selector: The best-effort selector captured from a UI element.
        tool: The target tool type.
        params: Generation parameters (text, duration, keys, button/clicks).
    ...
    match tool:
        case ToolType.CLICK:
            return [
                FlowNode(
                    tool="windows.click",
                    args=build_click_config(selector, button=params.button, clicks=params.clicks),
                ),
            ]
        ...
        case ToolType.KEYBOARD:
            return [
                FlowNode(
                    tool="windows.keyboard",
                    args=build_keyboard_config(params.keys),
                ),
            ]
        case ToolType.DELAY:
            return [
                FlowNode(
                    tool="windows.delay",
                    args=build_delay_config(params.duration_ms),
                ),
            ]
        case _:
            assert_never(tool)
...
def generate_action_config(
    ...
    match tool:
        case ToolType.CLICK:
            return build_click_config(selector, button=params.button, clicks=params.clicks)
        ...
        case ToolType.KEYBOARD:
            return build_keyboard_config(params.keys)
        case ToolType.DELAY:
            return build_delay_config(params.duration_ms)
        case _:
            assert_never(tool)
...
def build_inline_selector(
    ...
    *,
    ...
    """
    from r2flow.windows.selector_rank import control_type_display
...
def generate_nodes_from_config(
    ...
    come from :mod:`r2flow.windows.selector_rank` (minimal + verified
    ...
    match tool:
        case ToolType.CLICK:
            args = dict(config)
            if params.button in ("left", "right") and params.button != "left":
                args["button"] = params.button
            if params.clicks == 2:
                args["clicks"] = 2
            return [FlowNode(tool="windows.click", args=args)]
        case ToolType.INPUT_TEXT:
            return [
                FlowNode(
                    tool="windows.input_text",
...
def build_click_config(
    selector: BestSelector,
    *,
    button: str = "left",
    clicks: int = 1,
) -> dict[str, object]:
    """Build a ``windows.click`` config with button/clicks.

    Only non-default values are emitted so simple left-clicks stay clean.
    """
    config = build_inline_selector(selector)
    if button in ("left", "right") and button != "left":
...
def build_keyboard_config(keys: str) -> dict[str, str]:
    """Build a ``windows.keyboard`` config."""
    return {"keys": keys}


def build_delay_config(duration_ms: int) -> dict[str, int]:
    """Build a ``windows.delay`` config."""
    return {"duration_ms": max(1, int(duration_ms))}
