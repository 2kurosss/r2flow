...
if TYPE_CHECKING:
    from r2flow.windows.selector_rank import RankedSelector
...
class SeriesEvent:
    ...
    kind: Literal["stop", "mouse_down", "input", "key"]
    char: str | None = None
...
def _special_key_token(key: Any) -> str | None:
    """Map a pynput special key to a ``windows.keyboard`` bracket token."""
    if _kb is None:
        return None
...
def _hotkey_token(key: Any, *, ctrl: bool, shift: bool, alt: bool) -> str | None:
    """Build a ``windows.keyboard`` combo like ``[CTRL]S`` or ``[ALT][F4]``."""
    if _kb is None:
        return None
    base: str | None = None
...
def _series_listener(out: queue.Queue[SeriesEvent]) -> _kb.Listener:
    ...
    Events emitted:
        ...
        * ``SeriesEvent("key")`` — Enter/Tab/Esc/arrows/etc. and
          Ctrl/Alt hotkeys (``key="[CTRL]S"``), flushed as
          ``windows.keyboard`` nodes.
    """
    ...
    def on_press(key: Any) -> None:
        ...
        if ctrl and shift and key == _kb.Key.f2:
            out.put(SeriesEvent("stop", timestamp=_time.monotonic()))
            return
        now = _time.monotonic()
        # Hotkeys (Ctrl/Alt held) become keyboard nodes, not typed text.
        if ctrl or alt:
            token = _hotkey_token(key, ctrl=ctrl, shift=shift, alt=alt)
            if token is not None:
                out.put(SeriesEvent("key", key=token, timestamp=now))
            return
        # Standalone special keys flush pending text and become keyboard nodes.
        # Backspace stays a buffer edit; space stays typed text.
        special = _special_key_token(key)
        if special is not None and key not in (_kb.Key.space,):
            out.put(SeriesEvent("key", key=special, timestamp=now))
            return
        if _is_printable(key):
            out.put(SeriesEvent("input", _event_char(key), timestamp=now))
...
def _mouse_listener(out: queue.Queue[SeriesEvent]) -> _ms.Listener:
    ...
    Events emitted:
        * ``SeriesEvent("mouse_down")`` — any mouse button pressed,
          with button name, coordinates and timestamp.
    """
    import time as _time
    ...
    with _ListenerGroup([_shared_listener(events)]):
        ...
        while True:
            ...
            if event.kind == "trigger":
                ...
                params = GenerateParams()
                if tool is ToolType.KEYBOARD:
                    params = GenerateParams(keys=_prompt_keys())
                elif tool.needs_text:
...
def _prompt_keys() -> str:
    """Prompt the user for keys (mandatory for the keyboard tool).

    Returns:
        The entered keys string (guaranteed non-empty).
    """
    while True:
        try:
            raw = input("  -> keys ([ENTER], [TAB], [CTRL]S): ").strip()
        except (EOFError, KeyboardInterrupt):
            logger.info("Input cancelled; using [ENTER].")
            return "[ENTER]"
        if raw:
            return raw
        logger.error("Keys cannot be empty.")


def _prompt_duration() -> int:
    """Prompt the user for a duration in milliseconds (for wait/delay tools).
