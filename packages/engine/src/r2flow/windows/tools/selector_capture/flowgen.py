...
:func:`r2flow.windows.tools.selector_capture.record.record_series`). This
...
from r2flow.flow import FLOW_VERSION
...
def _label_for(node: FlowNode) -> str | None:
    ...
    for key in ("name", "automation_id", "text", "keys"):
        value = node.args.get(key)
        if isinstance(value, str) and value:
            return value[:_LABEL_LIMIT]
    duration = node.args.get("duration_ms")
    if isinstance(duration, int) and node.tool == "windows.delay":
        return f"delay {duration}ms"
    if node.tool == "windows.keyboard":
        return "keys"
    return None
