...
from r2flow.core.blocking import run_blocking
from r2flow.core.errors import InvalidInput, PlatformError
from r2flow.core.files import confine_path
...
# (R2FLOW_OCR_PATH / R2FLOW_OCR_LANG) — never interpolated into the
...
if ($env:R2FLOW_OCR_LANG -ne '') {
    $engine = [Windows.Media.Ocr.OcrEngine]::TryCreateFromLanguage(
        [Windows.Globalization.Language]::new($env:R2FLOW_OCR_LANG))
} else {
    $engine = [Windows.Media.Ocr.OcrEngine]::TryCreateFromUserProfileLanguages()
}
if ($null -eq $engine) { throw 'OCR engine unavailable for the requested language' }

$file = Await ([Windows.Storage.StorageFile]::GetFileFromPathAsync($env:R2FLOW_OCR_PATH)) `
    ...
    async def execute(self, config: dict[str, Any]) -> Any:
        ...
        try:
            ...
            if cleanup is not None:
                from r2flow.core.blocking import run_blocking as _rb
...
def _recognize(image_path: Path, language: str) -> str:
    ...
    env = dict(os.environ)
    env["R2FLOW_OCR_PATH"] = str(image_path)
    env["R2FLOW_OCR_LANG"] = language
    try:
