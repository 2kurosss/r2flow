...
#: ``R2FLOW_ALLOWED_COMMANDS="notepad.exe,calc.exe"``. An explicitly empty
#: value denies everything.
ENV_ALLOWLIST_VAR = "R2FLOW_ALLOWED_COMMANDS"
...
def _default_allowed_commands() -> frozenset[str]:
    """Default allowlist, overridden by ``R2FLOW_ALLOWED_COMMANDS`` when set."""
...
class ProcessTool(AbstractTool):
    ...
    Args:
        allowed_commands: Executables this instance may start. Defaults to
            ...
            ``R2FLOW_ALLOWED_COMMANDS`` env var (comma-separated) when set.
    """
