...
Usage:
  python -m r2flow.run_flow flow.json [--set NAME=VALUE ...] [--vars FILE] [--payload FILE]
  python -m r2flow.run_flow flow.json --tools my_tools.py
  python -m r2flow.run_flow flow.json --validate
  python -m r2flow.run_flow flow.json --transactional --queue NAME --db q.db
  python -m r2flow.run_flow flow.json --transactional --queue NAME --cloud URL --agent ID
...
Modes:
  ...
  --transactional        REFramework loop over a queue: each work item's
                         ...
                         --cloud/--agent (r2flow-cloud, token from the
                         R2FLOW_CLOUD_TOKEN env var)
...
from r2flow.core.errors import BusinessError, InfrastructureError
from r2flow.core.selectors import SelectorStore
from r2flow.flow import (
    FlowError,
    FlowRunner,
...
from r2flow.pack import manifest_lists_file
...
def _build_queue(args: argparse.Namespace) -> Any:
    from r2flow.core.queue import SqliteQueue
    ...
    if not args.cloud or not args.agent:
        message = (
            ...
            "or --cloud URL --agent ID (r2flow-cloud)"
        )
        raise SystemExit(message)
    from r2flow.core.http_queue import HttpQueue
...
def _seed_variables(doc: dict[str, Any]) -> dict[str, Any]:
    ...
    :func:`r2flow.flow.parse_typed_value` so ``3`` can stay a number and
    ``"3"`` a string.
    """
...
async def _run_transactional(
    ...
    *,
    ...
    from r2flow.core.transactions import run_transactions_async
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="r2flow.run_flow", description=__doc__)
    parser.add_argument(
        "flow",
        nargs="?",
        default=None,
    ...
    parser.add_argument("--cloud", default=None, help="r2flow-cloud API base URL")
    parser.add_argument("--agent", default=None, help="agent id for the cloud queue")
    parser.add_argument(
        "--token-env", default="R2FLOW_CLOUD_TOKEN", help="env var holding the queue token"
    )
    ...
    if args.pack:
        from r2flow.pack import load_manifest
    ...
    if args.validate:
        ...
        if store is None:
            store_path = os.environ.get("R2FLOW_SELECTOR_STORE", "selectors.json")
