...
Schema: ``r2flow-pack-v1``. The manifest is signature-ready: a future
...
PACK_SCHEMA = "r2flow-pack-v1"
TEMPLATE_FILE = "template.json"
TEMPLATE_SCHEMA = "r2flow-template-v1"
...
def fetch_pack(
    ...
    *source* is an ``http(s)://`` URL (e.g. served by r2flow-cloud) or a
...
def _main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="r2flow.pack", description="Build and verify bot packs (pack.json manifests)"
    )
    ...
    p_push.add_argument(
        "--api-url",
        default=os.environ.get("R2FLOW_API_URL"),
        help="orchestrator API root, e.g. https://host/api (default $R2FLOW_API_URL)",
    )
    p_push.add_argument(
        "--token-env",
        default="R2FLOW_API_TOKEN",
        help="env var holding the operator token (default R2FLOW_API_TOKEN)",
    )
    ...
    if args.command == "push":
        if not args.api_url:
            print(
                "no orchestrator URL: set --api-url or $R2FLOW_API_URL "
