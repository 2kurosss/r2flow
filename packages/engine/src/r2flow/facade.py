"""R2Flow — Facade for creating RPA bots with simple API."""
...
from r2flow.core.assets import AssetProvider, EnvAssetProvider
from r2flow.core.errors import ElementNotFound, InvalidInput
from r2flow.core.events import EventBus, Middleware, ToolEvent
from r2flow.core.redact import redact_text, redact_value
...
class R2Flow:
    ...
    def __init__(
        ...
        *,
        ...
        if dev_capture is None:
            dev_capture = os.environ.get("R2FLOW_DEV_CAPTURE", "").strip().lower() in (
                "1",
                "true",
                "yes",
            )
        ...
        if trace is not None:
            from r2flow.trace import FlowTracer
    ...
    def asset(self, name: str) -> str:
        ...
        default ``R2FLOW_ASSET_*`` environment variables). Every value
    ...
    async def _keyed_config(self, key: str | None, base: dict[str, Any]) -> dict[str, Any]:
        ...
        if not self._dev_capture:
            raise InvalidInput(
                ...
                "dev capture (R2FLOW_DEV_CAPTURE=1) to record it, or pass "
    ...
    async def screenshot(
        ...
        *,
        ...
        Requires ``mss`` and ``Pillow`` (included in ``r2flow[windows]``).
    ...
    async def clipboard(
        ...
        *,
        ...
        Requires ``pyperclip`` (included in ``r2flow[windows]``).
