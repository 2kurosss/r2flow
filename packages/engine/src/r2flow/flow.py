...
designer's debugger on top of :class:`FlowRunner`. Used standalone:
    ...
    from r2flow.flow import FlowRunner
...
from r2flow.core.errors import BusinessError, Cancelled, ElementNotFound, InfrastructureError
from r2flow.core.redact import redact_text
from r2flow.core.selectors import SelectorStore

if TYPE_CHECKING:
    from r2flow.core.assets import AssetProvider
    from r2flow.core.registry import ToolRegistry
...
def interpolate(
    ...
    *,
    ...
    (an :class:`~r2flow.core.assets.AssetProvider`); unknown assets raise,
...
class FlowRunner:
    ...
    Args:
        ...
        selector_store: Selector registry for ``key`` fields in tool
            configs (default: ``R2FLOW_SELECTOR_STORE`` env or
            ``selectors.json``).
        dev_capture: Interactive re-capture of missing/stale keys
            (default: ``R2FLOW_DEV_CAPTURE`` env).
    """
    ...
    def _store(self) -> SelectorStore:
        ...
        if self._selector_store is None:
            path = os.environ.get("R2FLOW_SELECTOR_STORE", "selectors.json")
    ...
    def _dev_capture_enabled(self) -> bool:
        ...
        return os.environ.get("R2FLOW_DEV_CAPTURE", "").strip().lower() in ("1", "true", "yes")
    ...
    async def _apply_selector_key(self, config: dict[str, Any]) -> str | None:
        ...
        if not self._dev_capture_enabled():
            raise FlowError(
                ...
                "record it with dev capture (R2FLOW_DEV_CAPTURE=1) or store it manually"
            )
        from r2flow.windows.tools.selector_capture import capture_once_async
    ...
    async def _run_subflow(self, node: dict[str, Any]) -> None:
        ...
        if path is not None:
            ...
            try:
                from r2flow.core.files import ENV_FILE_ROOT, confine_path
